...
MAX_REQUEST_QUEUE_LENGTH = config["max_request_queue_length"]   # Max length of the Request queue
POLICY = config["policy"]                                       # Scheduling policy for picking requests from buffer
QUANTUM_SIZE = config["quantum_size"]                           # Quantum size to run a process before context switch
NUM_USERS = int(num_users)                                      # Num of users at the client station
CONTEXT_SWITCH_OVERHEAD = config["context_switch_overhead"]     # Context Switch Overhead
STOPPING_CRITERION = config["stopping_criterion"]               # number of requests to be finished
...
for z in range(NUM_RUNS):
    ...
    request_queue = RequestQueue(MAX_REQUEST_QUEUE_LENGTH)      # An instance of Request queue
    ...
    while num_serviced < STOPPING_CRITERION:
        ...
        info_list.append(str("TIME" + "\t" + str(sim_time) + "\t" + str(core_handler.getUtilization()) +
                             "\t" + str(len(thread_list.list_of_threads_in_use) + len(request_queue.list_requests))))
